...
class LauncherState {
    ...
    val chatWorking: Boolean
        get() =
            chatPhase == ChatPhase.WAITING || chatPhase == ChatPhase.STREAMING ||
    ...
    /** Fetched conversation list (server-backed), newest first. */
    val madlenHistory = mutableStateListOf<com.r1.launcher.madlen.MadlenHeader>()
    var madlenListFocus by mutableIntStateOf(0)
    var madlenLoading by mutableStateOf(false)

    /** Turns of the open conversation. */
    val madlenMsgs = mutableStateListOf<com.r1.launcher.chat.ChatMsg>()
    var madlenId by mutableStateOf("")
    var madlenTitle by mutableStateOf("new chat")
    var madlenModel by mutableStateOf("")
    var madlenInput by mutableStateOf("")
    var madlenKbVisible by mutableStateOf(false)
    var madlenScrollTick by mutableIntStateOf(0)
    var madlenPinnedToBottom by mutableStateOf(true)
    var madlenWorking by mutableStateOf(false)
    var madlenError by mutableStateOf("")

    /** Live text of the in-flight assistant turn. */
    var madlenStreaming by mutableStateOf("")
    var madlenPhaseStartedAt by mutableStateOf(0L)
    var madlenScrollSeq by mutableIntStateOf(0)
    var madlenScrollDir by mutableIntStateOf(0)

    // login + settings mirrors
    var madlenUser by mutableStateOf("")
    var madlenPass by mutableStateOf("")
    var madlenBaseUrl by mutableStateOf("")
    var madlenLoggedIn by mutableStateOf(false)
    var madlenLoginBusy by mutableStateOf(false)
    var madlenLoginFocus by mutableIntStateOf(0)
    var madlenSettingsFocus by mutableIntStateOf(0)
    var madlenSpeak by mutableStateOf(false)
    var madlenTextSize by mutableIntStateOf(16)

    /** Which edit field the shared keyboard overlay is editing ("" = closed). */
    var madlenEditField by mutableStateOf("")
    var madlenEditInput by mutableStateOf("")

    /** Mic/transcription state for push-to-talk (mirrors the chat app). */
    var madlenPhase by mutableStateOf(ChatPhase.IDLE)

    /** Live partial transcript + level while the side button is held. */
    var madlenPartial by mutableStateOf("")
    var madlenMicLevel by mutableIntStateOf(0)

    fun openMadlenLogin() {
        madlenLoginFocus = 0
        madlenError = ""
        panel = Panel.MADLEN_LOGIN
    }

    fun openMadlenList() {
        madlenListFocus = 0
        panel = Panel.MADLEN_LIST
    }

    fun openMadlenChat() {
        madlenPinnedToBottom = true
        madlenScrollTick++
        panel = Panel.MADLEN_CHAT
    }

    fun openMadlenSettings() {
        madlenSettingsFocus = 0
        panel = Panel.MADLEN_SETTINGS
    }

    fun madlenResetError() {
        madlenError = ""
        madlenWorking = false
        madlenStreaming = ""
        madlenPhase = ChatPhase.IDLE
        madlenPartial = ""
        madlenMicLevel = 0
    }
    ...
    fun back() {
        panel =
            when (panel) {
                ...
                Panel.MADLEN_LOGIN -> Panel.APPS

                Panel.MADLEN_LIST -> Panel.APPS

                Panel.MADLEN_CHAT -> Panel.MADLEN_LIST

                Panel.MADLEN_SETTINGS -> Panel.MADLEN_CHAT
